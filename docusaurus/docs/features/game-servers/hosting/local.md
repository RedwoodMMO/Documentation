...
You can find all of the default configuration in `RedwoodBackend/config/node/default/realm/instances/redwoodBase.yaml` under `game-servers.local` variables, but here's an overview of what's available in the [Realm Instance Config](../../../configuration/realm-instance-config.md).
