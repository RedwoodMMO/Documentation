...
1. Copy the output folder (e.g. `LinuxServer`) to the `RedwoodBackend/dist/game-servers` folder (create it if it doesn't exist).
    - You can change the name of the folder to something else (e.g. `RedwoodBackend/dist/game-servers/LinuxServer-MyGame`); you will need to override the associated [Realm Instance Config](../../../configuration/realm-instance-config.md) `game-servers.local-dir` for the realm you're configuring.
