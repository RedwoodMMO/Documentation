...
1. Update your project to use the `Login with Steam` function instead of the normal `Login` function.
    - This function has the same auth update callback/delegate as the normal `Login` function.
    - You can use the `LoginWithSteam` latent Blueprint node that uses `RedwoodClientGameSubsystem`
    - You can also use the C++ function:
        1. Add the `RedwoodSteam` module to your `.Build.cs` file
        1. Include `RedwoodClientInterface.h`, `RedwoodClientGameSubsystem.h`, and `RedwoodSteamClientInterface`
        1. Get a reference to the active Redwood Client interface:
            ...
            ```
        1. Call `LoginWithSteam`; be sure to replace `OnUpdate` below as applicable to handle the auth callback:
            ``` c++
            URedwoodSteamClientInterface::LoginWithSteam(ClientInterface, FRedwoodAuthUpdateDelegate OnUpdate);
            ```
1. Update your UE project's `Config/DefaultEngine.ini` to enable the Steam online subsystem, but making sure to disable Steam networking:
