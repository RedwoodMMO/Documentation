...
- Matchmaking
  - [**Simple Match**](./matchmaking/simple-match.md) - A dead-simple emulator for Open Match used in development without Kubernetes; shares some config with Open Match
  - [**Open Match**](./matchmaking/open-match.md) - An open-source, flexible, and scalable matchmaker
- [**Queuing**](./queuing.md) - This is used in for lobby-based systems where players already know what server they want to join, but it may be full (i.e. MMORPGs). This provider implements a FIFO (First-In-First-Out) queue system.
- [**Custom Flow**](./custom.md) - This enables you to program a custom ticketing flow for more advanced setups.
...
Our `kubernetes` [inheritable config environment](../../configuration/overview.md#config-environment-inheritance) overrides this variable for you to `open-match` in `RedwoodBackend/config/node/kubernetes/realm/instances/default.yaml`.
...
You may also need to implement your own ticketing provider for your specific game; if you have Dedicated Support, [reach out](../../support/how-to-get-support.md) and we'll help discuss your options!
