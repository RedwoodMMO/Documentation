...
Each player has 1 `PlayerIdentity` entry stored in the [Director](../../../architecture/overview.md#director) database. This is the first entry point usually via [authentication](../../authentication/overview.md). There's no game data stored for the player here.
...
This stores any data related to the progress of the character; some examples include:
- Achievements
- Quests
- NPC relationships
- Skill trees
- Unlocked gear, skins, and game modes

There are no recommendations whether this is replicated or not.

:::info
This field is disabled by default in the `URedwoodCharacterComponent` and must explicitly be enabled by setting `URedwoodCharacterComponent::bUseProgress` to `true` (either via C++ or the BP defaults interface).
:::

#### `abilitySystem`

This stores any data related to abilities. This DB field can be used by both the `URedwoodCharacterComponent` with your own data (disabled by default) or by using the optional built-in [Gameplay Ability System (GAS) integration](./gas.md).
...
This stores data used by the Redwood system for other features (e.g. [zone transferring](../../game-servers/zones.md)). Altering this object can cause other systems to break.

## Unreal Setup

:::note
While using the `URedwoodCharacterComponent` described below is **definitely recommended**, it's not required and may get in the way of integrating some existing systems (especially those that heavily use SaveGames for persisting character data).

Character data is available in `URedwoodPlayerStateComponent::RedwoodCharacter` (only on the server; not replicated for you). You will get raw `USIOJsonObject` objects for the various data fields, and you will need to deserialize it as you see fit.

You can call `URedwoodPlayerStateComponent::MarkCharacterDataDirty` to persist _all_ of the data in the struct. It's **highly not recommended to mix this method with the `URedwoodCharacterComponent` and will result in undefined behavior**.
:::
...
1. Your PlayerStateClass in the game mode class needs to have an instance of the `URedwoodPlayerStateComponent` Actor Component attached
1. Your DefaultPawnClass in the game mode class needs to have an instance of the `URedwoodCharacterComponent` Actor Component attached
1. Create a new struct for [each of types](#playercharacter) of data (**except** `redwoodData`)
    ...
    - **NOTE:** Each struct **must** have a property of type `Integer` or `int32` with the name `SchemaVersion` that has a default value that is **non-negative** (default starts at `0`)
...
Players create a `PlayerCharacter` when they're not connected to a game server. They do this by calling the `URedwoodClientGameSubsystem::CreateCharacter` function with the character's `name` and `characterCreatorData`. The [other fields](#playercharacter) are technically set to empty objects when this happens (no `schemaVersion` or any other variable).

However, most games will want to set these other fields to some initial data; for example, you may want all characters to have `xp` and `level` fields in `metadata` initialized to `0` and `1` respectively. You can't store these variables in `characterCreatorData` because players can exploit this and set their level to whatever they want, but Redwood's data system was designed so that the backend doesn't need to understand the schema of your game data.

:::info
In most cases, [`URedwoodSyncComponent`](../../sync-items.md#persistent-items) instances will be spawned by the server and their `Data` structs will be initialized during spawning with the default values. The `Creation` functions won't be called on those cases.
:::

Due to all this, Redwood initializes the other structures when that `PlayerCharacter` first connects to any game server. The server sees that there's no `schemaVersion` in each of the fields, so it needs to be initialized. This is what happens in order:
1. The default values of each of the variables of your structs are inserted into the struct (BP or C++)
1. Redwood will call the `<VariableName>_Creation` _if it exists_. This is an optional function that has **no inputs or output**. This function is useful if you need to do extra calculations or logic during creation of new characters. You can access and modify the class's respective member variable as needed. Similar to other [migration functions](#changing-the-structures), this function should exist on the Actor that the `URedwoodCharacterComponent` is attached to unless you changed the `bStoreDataInActor` value to `false`, in which case the function should be added to the child class you created of

    :::note
    A creation function can exist for each variable (`Metadata_Creation`, `EquippedInventory_Creation`, `NonequippedInventory_Creation`, `Data_Creation`) and each will be called during that first connection.
    :::
...
Migration functions should be stored in the actor the `URedwoodCharacterComponent` or `URedwoodSyncComponent` are attached to, unless you changed the value of `bStoreDataInActor` to `false`. In that case, the functions (like the data) should be stored in the component instance.
...
:::warning
When you're changing the structures for a live game that won't have a database wipe, you need to change the associated `Latest<VariableName>SchemaVersion` variable in your character class via the Class Defaults/C++ constructor. This should always be an always-increasing **non-negative** integer (default starts at `0`).
:::
